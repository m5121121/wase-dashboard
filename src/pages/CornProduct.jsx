...
const CornProduct = () => {
  ...
  /**
   * 最高・最低気温ラベルを確実に描画するカスタムコンポーネント
   */
  const CustomLabelLayer = (props) => {
    // Rechartsから渡される描画座標データ(points)を利用
        ...
        {maxP && (
          <g>
            ...
            <text 
              ...
              y={maxP.y} 
              dy={-15} 
              // 右端に寄りすぎている場合は左にオフセットをかける
              dx={maxP.x > width * 0.9 ? -8 : 8}
              fill="#f43f5e" 
              fontSize={16} 
              fontWeight="900"
              textAnchor={maxP.x > width * 0.9 ? "end" : "start"}
            >
              最高 {maxVal}℃
            </text>
          </g>
        ...
        {minP && (
          <g>
            ...
            <text 
              ...
              y={minP.y} 
              dy={25} 
              dx={minP.x > width * 0.9 ? -8 : 8}
              fill="#0ea5e9" 
              fontSize={16} 
              fontWeight="900"
              textAnchor={minP.x > width * 0.9 ? "end" : "start"}
            >
              最低 {minVal}℃
            </text>
          </g>
          ...
          <div style={{ width: '100%', height: '400px' }}>
            {chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 35, right: 0, left: -25, bottom: 0 }}>
                  <defs>
